...
async def _async_pricing_fetcher(
    endpoint_id: str,
    authorization: str,
) -> httpx.Response:
    return _HTTP_RESPONSE_ADAPTER.validate_python(
...
class FalAIVideoConfig(BaseVideoConfig):
    ...
    def _track_sync_provider_cost(
        ...
        try:
            ...
            verbose_logger.warning(
                "Unable to track fal.ai provider cost: %s",
                type(exc).__name__,
            )
    ...
    def _cost_context(
        ...
        encoded_video_id = logging_obj.optional_params.get("video_id")
        ...
        if not isinstance(request_id, str) or not request_id:
            return None
        if not authorization or raw_billable_units is None:
            return None
        try:
    ...
    def _fetch_sync_price(
        ...
        response = self._sync_pricing_fetcher(
            context.endpoint_id,
    ...
    async def _fetch_async_price(
        self,
        context: _FalCostContext,
    ) -> _FalPrice | None:
        response = await self._async_pricing_fetcher(
            context.endpoint_id,
            context.authorization,
        )
        response.raise_for_status()
    ...
    def _cache_valid_price(
        ...
        price = next(
            (item for item in pricing.prices if item.endpoint_id == context.endpoint_id),
            None,
        )
